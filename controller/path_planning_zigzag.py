...
class PathPlannerZigZag:
    ...
    def __init__(self, coverage_width, coverage_height, waypoint_spacing, start_x, start_y, scoop_width, overlap_percentage: int):
    ...
    def get_number_of_horizontal_passes(self):
        return round(((self.overlap_percentage / 100) + 1) * (self.coverage_height / self.scoop_width))
    ...
    def get_x_coordinates_for_vertcal_passes(self):
        ...
        for i in range(num_vertical_passes):
            x_coordinates.append(float(self.coverage_width) + self.start_x if right_vertical_segment else self.start_x)
    ...
    def generate_zigzag_path(self):
        ...
        for y_idx in range(1, len(y_coordinates)):
            ...
            self.paths.append(self.generate_horizontal_segment(y_coordinates[y_idx - 1], start_x, end_x, x_forward))
            x_forward = not x_forward
            self.paths.append(self.generate_vertical_segment(end_x, y_coordinates[y_idx - 1], y_coordinates[y_idx], y_forward))
            start_x, end_x =  end_x, start_x
            self.paths.append(self.generate_horizontal_segment(y_coordinates[y_idx], start_x, end_x, x_forward))
